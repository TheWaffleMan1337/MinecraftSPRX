...
void AddNewButton_Stub(uint32_t r3, font_t* r4, uint32_t r5, uint32_t r6) {
	__nop(); __nop(); __nop(); __nop(); __nop();
}
void AddNewButton_Hook(uint32_t r3, font_t* r4, uint32_t r5, uint32_t r6) {

	

	AddNewButton_Stub(r3, r4, r5, r6);
}
...
extern "C" int _MCStreamSPRX_prx_entry(void) {
	...
	*(uint32_t*)(0x0140A3B8) = (uint32_t)(onEntityUpdate_Hook);
	...
	HookFunctionStart(0x00875204, *(uint32_t*)(AddNewButton_Hook), *(uint32_t*)(AddNewButton_Stub));


	SceNpBasicPresenceDetails2 pres;
	memset(&pres, 0, sizeof(SceNpBasicPresenceDetails2));
	pres.struct_size = sizeof(SceNpBasicPresenceDetails2);
	strcpy(pres.status, "Woah this works??");

	sceNpBasicSetPresenceDetails2_Stub(&pres, 2);
...
extern "C" int _MCStreamSPRX_prx_exit(void) {
	...
	*(uint32_t*)(0x0140A3B8) = (uint32_t)(0x014A1C10);
	...
	UnHookFunctionStart(0x00875204, *(uint32_t*)(AddNewButton_Stub));
